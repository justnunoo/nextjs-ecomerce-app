...
export async function GET() {
    ...
    try {
        ...
        if (!userCart) {
            return new Response(JSON.stringify({ message: "Cart not found", cartEmpty: true }), {
                status: 404,
        ...
        return new Response(JSON.stringify({ message: `Error placing the order: ${error.message}`, cartEmpty: false }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
        });
    }
